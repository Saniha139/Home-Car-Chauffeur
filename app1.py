...
def go_online():
    ...
    with get_db() as db:
        ...
        # Fetch pending rides
        rides = db.execute('SELECT * FROM booking WHERE status = ?', ('waiting',)).fetchall()
...
def ride_action(ride_id, action):
    ...
    with get_db() as db:
        ...
        if action == 'accept':
            db.execute('UPDATE booking SET status = ?, driver_id = ? WHERE id = ?', ('accepted', driver_id, id))
        elif action == 'reject':
            db.execute('UPDATE booking SET status = ? WHERE id = ?', ('rejected', id))
        db.commit()
